...
let ind ?(ofs = 0) ?index ?scale r fmt () =
  match (index, scale) with
  | None, None -> fprintf fmt "%d(%s)" ofs r
  | Some r1, Some scale -> fprintf fmt "%d(%s,%s,%d)" ofs r r1 scale
  | _ -> raise (Invalid_argument "you must specify both index and scale")

let abslab (l : label) fmt () = fprintf fmt "%a" mangle l
let rellab (l : label) fmt () = fprintf fmt "%a(%%rip)" mangle l
let lab = abslab
...
(* nom de fonctions compréhensible par un être humain on est en 2022 et on a
   toujours pas compris ce que disait Grace Hopper dont le portrait est dans la salle de TD
